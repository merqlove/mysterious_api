Rails.application.configure do
  ...
  config.cache_store = :dalli_store, ENV.fetch('MEMCACHED_SERVERS') { 'localhost' }, {
                                       namespace: "mysterious:#{Rails.env}",
                                       expires_in: 1.day,
                                       compress: true }

  # Use a real queuing backend for Active Job (and separate queues per environment)
  # config.active_job.queue_adapter     = :resque
  config.active_job.queue_name_prefix = "mysterious_#{Rails.env}"
